...
| `block.py`            | `block_lookup_func(start, end, all_blocks, step)`             | Casts a ray and returns all blocks intersected by it. Used for occlusion/light tracing. |
...
| `lighting.py`         | `calculate_emission_from_nearby_emitters(block, all_blocks)`  | Sums emissive light contribution from nearby emitters.                                  |
